...
define([
  ...
  const CapacityAnalysis = Accessor.createSubclass({
    ...
    properties: {
      ...
      infoPanel: { type: HTMLDivElement },
      layerTitleNode: { type: HTMLDivElement },
      indicatorNode: { type: HTMLDivElement },
      indicatorSVG: { type: SVGElement },
      bedsTotalNode: { type: HTMLDivElement },
      percentCapacityNode: { type: HTMLDivElement },
      viewPanel: { type: HTMLDivElement },
      loaderNode: { type: HTMLDivElement },
      map: {
        type: EsriMap
      },
      viewProperties: {
      ...
      days: {
        ...
        set: function(days){
          this._set('days', days);
          this.layerAnalysisResults = new CapacityAnalysisResults({ title: this.title, days });
      ...
      return promiseUtils.create((resolve, reject) => {
        ...
        watchUtils.whenDefinedOnce(this, 'viewPanel').then(() => {
          ...
          this.mapView.when(() => {
            ...
            // HOME //
            const home = new Home({ view: this.mapView });
    ...
    _getAnalysisResults: function(){
      return promiseUtils.create((resolve, reject) => {
        ...
        dataQuery.set({ where: '1=1', outFields: this.layerView.availableFields });
        this.layerView.queryFeatures(dataQuery).then(dataFS => {
          ...
          if(dataFS.features.length){
            ...
            dataFS.features.forEach(feature => {
              ...
              this.featureAnalysisResults[featureId] = new CapacityAnalysisResults({ title: featureTitle, days: this.days });
     ...
     * @param units
     */
    updateCounts: function({ count, capacity, units }){
    ...
    initializeAnalysisDetails: function(){
      ...
      const analysisDetails = { title: this.title, description: this.description };
